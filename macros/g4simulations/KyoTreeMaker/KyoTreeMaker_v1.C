...
KyoTreeMaker::KyoTreeMaker(const std::string &name, bool fill_g4particle_tree, bool fill_track_tree, bool fill_tower_tree, bool fill_cluster_tree, bool fill_truthjet4_tree, bool fill_towerjet4_tree, bool fill_otherjetR_tree, bool fill_bhhit_tree) : SubsysReco()
{
  ...
  //
  _fill_truthjet2_tree=fill_otherjetR_tree; //temp
  _fill_truthjet3_tree=fill_otherjetR_tree; //temp
  _fill_truthjet4_tree=fill_truthjet4_tree; 
  _fill_truthjet5_tree=fill_otherjetR_tree; //temp
  //
  _fill_towerjet2_tree=fill_otherjetR_tree; //temp
  _fill_towerjet3_tree=fill_otherjetR_tree; //temp
  _fill_towerjet4_tree=fill_towerjet4_tree;
  _fill_towerjet5_tree=fill_otherjetR_tree; //temp
  //
...
int KyoTreeMaker::Init(PHCompositeNode *topNode)
{
  ...
  if (_fill_cemc_cluster_tree){
    ...
    //cemc_cluster_tree->Branch("cl_cemc_n", &_b_cl_cemc_n, "cl_cemc_n/I");
...
/*
    cemc_cluster_tree->Branch("cl_cemcMatched_e", &_b_cl_cemcMatched_e, "cl_cemcMatched_e/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_p", &_b_cl_cemcMatched_p, "cl_cemcMatched_p/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_pt", &_b_cl_cemcMatched_pt, "cl_cemcMatched_pt/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_eta", &_b_cl_cemcMatched_eta, "cl_cemcMatched_eta/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_phi", &_b_cl_cemcMatched_phi, "cl_cemcMatched_phi/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_id", &_b_cl_cemcMatched_id, "cl_cemcMatched_id/I");
    cemc_cluster_tree->Branch("cl_cemcMatched_ntwr", &_b_cl_cemcMatched_ntwr, "cl_cemcMatched_ntwr/I");
    cemc_cluster_tree->Branch("cl_cemcMatched_prob", &_b_cl_cemcMatched_prob, "cl_cemcMatched_prob/F"); //only for CEMC
    cemc_cluster_tree->Branch("cl_cemcMatched_chi2", &_b_cl_cemcMatched_chi2, "cl_cemcMatched_chi2/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_track_p", &_b_cl_cemcMatched_track_p, "cl_cemcMatched_track_p/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_track_deta", &_b_cl_cemcMatched_track_deta, "cl_cemcMatched_track_deta/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_track_dphi", &_b_cl_cemcMatched_track_dphi, "cl_cemcMatched_track_dphi/F");
    cemc_cluster_tree->Branch("cl_cemcMatched_track_dr", &_b_cl_cemcMatched_track_dr, "cl_cemcMatched_track_dr/F");
*/
  }
...
int KyoTreeMaker::process_event(PHCompositeNode *topNode)
{
  ...
  if (_fill_track_tree){
    ...
    for (SvtxTrackMap::ConstIter trk_iter =trackmaps->begin(); trk_iter != trackmaps->end(); trk_iter++){
      ...
      //_b_cemc_clDeta.push_back( (float)track->get_cal_deta(SvtxTrack::CEMC) ); not for cluster!!
  ...
  if (_fill_cemc_cluster_tree){
    ...
    for ( RawClusterContainer::ConstIterator cluster_iter = clrange_cemc.first; cluster_iter != clrange_cemc.second; ++cluster_iter ) {
      ...
      float z = z0 - _b_vtx_z;
      float eta = asinh(z/r); // eta after shift from vertex
      //float eta = cl_cemc->get_eta(_b_vtx_z);
  ...
  if (_fill_ihcal_cluster_tree){
    ...
    for ( RawClusterContainer::ConstIterator cluster_iter = clrange_ihcal.first; cluster_iter != clrange_ihcal.second; ++cluster_iter ) {
      ...
      float z = z0 - _b_vtx_z;
      float eta = asinh(z/r); // eta after shift from vertex
      //float eta = cl_ihcal->get_eta(_b_vtx_z);
  ...
  if (_fill_ohcal_cluster_tree){
    ...
    for ( RawClusterContainer::ConstIterator cluster_iter = clrange_ohcal.first; cluster_iter != clrange_ohcal.second; ++cluster_iter ) {
      ...
      float z = z0 - _b_vtx_z;
      float eta = asinh(z/r); // eta after shift from vertex
      //float eta = cl_ohcal->get_eta(_b_vtx_z);
...
void KyoTreeMaker::getClusterByIndex( PHCompositeNode *topNode, std::string detName, unsigned int cl_id, float teta, float tphi){
  ...
  float dphi = -531;
  //float dr = -531;
  //std::cout << "track teta = " << teta << std::endl;
  //std::cout << "track tphi = " << tphi << std::endl;
  if((cl_id>=0) && (cl_id<clusters->size())){
    RawCluster *cluster = clusters->getCluster(cl_id);
    ...
    z = z0 - _b_vtx_z;
    eta = asinh(z/r);
    //std::cout << "*** " << clusternodename.c_str() << std::endl; 
    //std::cout << "cluster eta = " << eta << std::endl;
    //std::cout << "cluster phi = " << phi << std::endl;
    deta = eta-teta;
    dphi = phi-tphi;
